...
import { SignInWithAppleAuthorization, SignInWithAppleCredential, SignInWithAppleOptions, SignInWithAppleState } from "./index";
...
export function signInWithApple(options?: SignInWithAppleOptions): Promise<SignInWithAppleAuthorization> {
  return new Promise<any>((resolve, reject) => {
    ...
    controller = ASAuthorizationController.alloc().initWithAuthorizationRequests(iOSNativeHelper.collections.jsArrayToNSArray([request]));
  ...
  authorizationControllerDidCompleteWithAuthorization(
    ...
    if (authorization && authorization.credential) {
      ...
      if (authorization.credential.authorizedScopes) {
        data.credential.authorizedScopes = iOSNativeHelper.collections.nsArrayToJSArray(
